...
func NewAPIServerCommand() *cobra.Command {
	// 加入一些默认的配置，比如准入机制的 PodPreset 机制
...
// 函数名称里的 ServerChain 怎么理解呢？
// 从下面代码可以看到，先是创建了 apiExtensionServer，然后创建 kubeAPIServer 的时候将  apiExtensionsServer.GenericAPIServer 作为参数传递了进去
// 然后创建 aggregatorServer 时，将 kubeAPIServer.GenericAPIServer 作为参数传递了进去
// 所以这三个服务形成了一个链
func CreateServerChain(completedOptions completedServerRunOptions, stopCh <-chan struct{}) (*aggregatorapiserver.APIAggregator, error) {
	...
	// API核心服务，包括常见的 Pod、Deployment、Service
...
func CreateKubeAPIServerConfig(
	...
	*genericapiserver.DeprecatedInsecureServingInfo,
	...
	genericConfig, versionedInformers, insecureServingInfo, serviceResolver,
		pluginInitializers, admissionPostStartHook, storageFactory, err := buildGenericConfig(s.ServerRunOptions, proxyTransport)
	if err != nil {
...
// 构建通用配置。这个方法能接受一些选项，然后创建一个通用配置并返回
func buildGenericConfig(
	...
	// storageFactory 是一个存储工厂，K8s 可以使用 ETCD 作为存储实现这个工厂接口，也可以使用其他存储来实现
	// 这里就是创建一个相关的配置
...
func BuildAuthorizer(s *options.ServerRunOptions, EgressSelector *egressselector.EgressSelector, versionedInformers clientgoinformers.SharedInformerFactory) (authorizer.Authorizer, authorizer.RuleResolver, error) {
	...
	// 和 authentication 的实例化一样，在 New 方法里
