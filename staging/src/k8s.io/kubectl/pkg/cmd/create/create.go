...
func NewCmdCreate(f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	// 添加 -f 的选项，保存到 o.FilenameOptions.Filenames 中
	...
	// create subcommands
	// 创建 create 下面的子命令，比如 kubectl create namespace/quota/secret、configMap...
...
func (o *CreateOptions) RunCreate(f cmdutil.Factory, cmd *cobra.Command) error {
	...
	// 通过 Builder 模式将 yaml 文件、或命令行获得的信息转化为一系列的资源
	// NewBuilder() 返回一个资源构建器，用于结构化 api 对象
	r := f.NewBuilder().
		// Unstructured 为 r 添加了 RESTful Client，使它能够发送数据到 kube-apiserver
		Unstructured().
		Schema(schema).
		ContinueOnError().
		// 默认为 ns 为 default
		NamespaceParam(cmdNamespace).DefaultNamespace().
		// 读取文件信息，除了支持本地文件，还支持标准输入流，以及 http/https 可访问的文件
		// 并且设置了 FileVisitor，用于下面的 r.Visit调用
	...
	err = r.Visit(func(info *resource.Info, err error) error {
		...
		if o.DryRunStrategy != cmdutil.DryRunClient {
			...
			// kubectl 向 kube-apiserver 发送请求
			obj, err := resource.
				NewHelper(info.Client, info.Mapping).
				DryRun(o.DryRunStrategy == cmdutil.DryRunServer).
				WithFieldManager(o.fieldManager).
				// 这里的 info.Object 是 runtime.Object 类型，是通过 FileVisitor 解析获取的
				// 而解析器的设置是在 NewBuilder 的 Unstructured 阶段
				// 所以要看 info.Object 的获取过程，要去看 FileVistior 的 Visit 方法
